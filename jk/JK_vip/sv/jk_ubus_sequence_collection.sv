...
class error_seq extends jk_ubus_base_sequence;
 ...
 virtual task body();
	jk_ubus_master_transfer tr;
  `uvm_info(get_type_name(), $sformatf("&s starting error_sequence",get_sequence_path()), UVM_MEDIUM)

//  `uvm_do_with(tr, {read == 1; write == 1;})
 `uvm_create(tr)
 start_item(tr);
 tr.randomize();
 tr.read = 1;
