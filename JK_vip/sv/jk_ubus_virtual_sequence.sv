class jk_ubus_virtual_sequence extends uvm_sequence;
 ...
 virtual task body();
	...
	m_error_seq = error_seq::type_id::create("m_error_seq");
	if (!m_error_seq.randomize())
	 `uvm_fatal("error_sequence", "sequence randomization failed")
//	m_incr_rwr_seq = incr_read_write_read_seq::type_id::create("m_incr_rwr_seq");
 ...
 join_none
 m_error_seq.start(p_sequencer.master_sequencer);
